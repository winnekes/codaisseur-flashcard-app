...
let questionsData = [
    ...
    },
    {
        ...
        answer:
            'Following are looping structures in Javascript: for while, do-while',
        category: 'JS'
    },
    {
        id: 6,
        question:
            'Explain the difference between <em>"=="</em> and <em>"==="</em>?',
        answer:
...
function clearLocalUserStorage() {
    confirmedClear = confirm(
        'Are you sure you want to delete all the flashcards you added?'
    );
    if (confirmedClear) {
        userStorage.clear();
        jumpToAnchor('top');
...
function displayCategories() {
    for (let i = 0; i < categories.length; i++) {
        ...
        categoryButton.setAttribute(
            'onclick',
            "displayRandomFlashcard('" + categories[i] + "');"
        );
...
function displayRandomFlashcard(category = false) {
    ...
    if (category) {
        ...
        newFlashcardButton.setAttribute(
            'onclick',
            "displayRandomFlashcard('" + category + "')"
        );
        ...
        if (filteredQuestions === undefined || filteredQuestions.length == 0) {
            alert(
                "There are no flashcards in this category. Why don't you add some?"
            );
        } else {
            ...
            deleteFlashcardButton.setAttribute(
                'onclick',
                "deleteFlashcard('" + filteredQuestions[randNum].id + "')"
            );
        }
    } else {
        ...
        deleteFlashcardButton.setAttribute(
            'onclick',
            "deleteFlashcard('" + questions[randNum].id + "')"
        );
        newFlashcardButton.setAttribute('onclick', 'displayRandomFlashcard()');
    }
...
function deleteFlashcard(flashcardID) {
    const confirmedDelete = confirm(
        'Are you sure you want to delete the current flashcard?'
    );
    if (confirmedDelete) {
        for (let i = 0; i < questions.length; i++)
